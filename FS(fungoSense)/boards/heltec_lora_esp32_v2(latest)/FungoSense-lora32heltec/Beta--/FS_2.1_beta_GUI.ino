//DADOS:
//*********************************************************************************************************
char ssid[] = ""; // nome da rede wifi
char pass[] = ""; //senha do wifi
#define BLYNK_TEMPLATE_ID           "" // id do template
#define BLYNK_TEMPLATE_NAME         "" // nome do template
#define BLYNK_AUTH_TOKEN            "" // token do blynk
float tmaxf = 28; // escolha a temperatura que a ventilação vai ligar caso seja reiniciado
int refg = 1; // escolha de 1 a 0 que vai definir se a ventilação vai está liga caso seja reiniciado
//*********************************************************************************************************



#include <heltec.h>
#include <BlynkSimpleEsp32.h>
#include <WiFi.h>
#include <WiFiClient.h>
#include <DHT.h>
#include <Wire.h>
#include <ESPAsyncWebServer.h>

int relayPin = 17; // Pino do relé
#define BLYNK_PRINT Serial
#define DHTPIN 16 // Pino de conexão do sensor DHT11
#define DHTTYPE DHT11 // Tipo do sensor DHT (DHT11 neste caso)
DHT dht(DHTPIN, DHTTYPE);
BlynkTimer timer;

#define OLED_WIDTH 128
#define OLED_HEIGHT 64
AsyncWebServer server(80);

String thtml;
String hhtml;
String luzhtml;


void setup() {
  Serial.begin(115200); // Inicializa a comunicação serial
  
  //Inicio:

  pinMode(relayPin, OUTPUT);
  digitalWrite(relayPin, LOW);

  Heltec.begin(true /* DisplayEnable */, false /* LoRaEnable */, true /* SerialEnable */);
  Heltec.display->init();
  Heltec.display->clear();

  int centerbeginX = Heltec.display->width() / 2;
  int centerbeginY = Heltec.display->height() / 2;
  int radiusbegin = 20;

  Heltec.display->clear();
  Heltec.display->drawCircle(centerbeginX, centerbeginY, radiusbegin);
  Heltec.display->display();
  delay(1000);

  Heltec.display->clear();
  const unsigned char image1[] = {
// 'espacoslogoblack', 128x64px

  };
  const unsigned char image2[] = {
// 'fungosenseblacj', 128x64px

  };

  Heltec.display->drawXbm(0, 0, OLED_WIDTH, OLED_HEIGHT, image1);
  Heltec.display->display();
  delay(3000);
  Heltec.display->clear();
  
  Heltec.display->drawXbm(0, 0, OLED_WIDTH, OLED_HEIGHT, image2);
  Heltec.display->display();

  delay(2000);
  dht.begin(); // Inicializa o sensor DHT11
          
  timer.setInterval(500L, sensor);

  Blynk.begin(BLYNK_AUTH_TOKEN, ssid, pass);
  //leitura de status de conexão
        if (WiFi.status() == WL_CONNECTED) {
          Heltec.display->clear();
          Heltec.display->setFont(ArialMT_Plain_10);
          Heltec.display->setTextAlignment(TEXT_ALIGN_LEFT);
          Heltec.display->drawString(0,0, "wifi conectado a " + String(ssid));
          Heltec.display->display();
          delay(1000);
        } 
}

void loop() {
  Blynk.run();
  timer.run();

}

void sensor(){
  while(true){
  delay(2000); // Intervalo de leitura dos dados (2 segundos neste caso)

  int h = dht.readHumidity(); // Lê a umidade do sensor
  float t = dht.readTemperature(); // Lê a temperatura do sensor em graus Celsius

  thtml = t;
  hhtml = h;

  Heltec.begin(true /* DisplayEnable */, false /* LoRaEnable */, true /* SerialEnable */);
  Heltec.display->clear();
  Heltec.display->setFont(ArialMT_Plain_16);
  Heltec.display->setTextAlignment(TEXT_ALIGN_LEFT);
  Heltec.display->drawString(40,10, String(t) + "°c");
  Heltec.display->setFont(ArialMT_Plain_10);
  Heltec.display->drawString(20,30, String(h) + "%");
  Heltec.display->display();

  Serial.print("Umidade: ");
  Serial.print(h);
  Serial.print("%\t"); // Tabulação
  Serial.print("Temperatura: ");
  Serial.print(t);
  Serial.println("°C");
  Serial.print("temperatura maxima definida: ");
  Serial.print(tmaxf);
  Serial.println(" °C");

  Blynk.virtualWrite(V6, t);
  Blynk.virtualWrite(V5, h);

  if (refg ==1){
    Heltec.display->drawString(20,0, "REFG ativa> " + String(tmaxf) + "°c");
    Heltec.display->display();

    if(t >= tmaxf){
      digitalWrite(relayPin, HIGH);
    }
    else{
      digitalWrite(relayPin, LOW);
    }
  }
  if (refg ==0){
    digitalWrite(relayPin, LOW);
  }
  
int ldralt = 0;
int ldrValue = analogRead(32);

    Serial.println(ldrValue);

    float percentage = (ldrValue * 100.0) / 4095.0;
    Heltec.display->drawString(20,45, String(percentage) + "%");
luzhtml = percentage;
    Heltec.display->display();
const unsigned char image3interface[] = {
// '20231018_143403_0000', 128x64px

  };
  Heltec.display->drawXbm(0, 0, OLED_WIDTH, OLED_HEIGHT, image3interface);
  Heltec.display->display();

  Blynk.virtualWrite(V2, percentage);

  // Conectar-se à rede Wi-Fi
  WiFi.begin(ssid, pass);
  while (WiFi.status() != WL_CONNECTED) {
    delay(1000);
    Serial.println("Conectando ao WiFi...");
  }
  Serial.println("Conectado à rede Wi-Fi");

  // Rota para a página inicial
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/html", getPage());
  });

    // Rota SSE para atualizar a variável1
  server.on("/sse", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/event-stream", getUpdate());
  });

  // Iniciar o servidor
  server.begin();

}
}

BLYNK_WRITE(V0) { //interruptor de refrigeração
  int valuev0 = param.asInt();
refg = valuev0;
}

BLYNK_WRITE(V1) { //escolha de temperatura
  float valuev1 = param.asInt();
tmaxf = valuev1;
}


//html control
String getPage() {

  String page = "<html><head><meta charset=\"UTF-8\"><style>";
  page += "body { color: #C0C0C0; background-color: black; font-family: Arial, Helvetica, sans-serif;}";
  page += "h1 {color: white;}";
  page += "";
  page += "</style></head><body>";
  page += "<h1>FungoSense</h1>";
  page += "<p>LocalServer FungoSense.</p>";
  page += "<h3>Temperatura: <span id='thtml'>" + String(thtml) + "</span>C°</h3>";
  page += "<script>var source = new EventSource('/sse'); source.onmessage = function(event) {document.getElementById('thtml').innerText = event.data1;};</script>";
  
  page += "<h3>Umidade: <span id='hhtml'>" + String(hhtml) + "</span>%</h3>";
  page += "<script>var source = new EventSource('/sse'); source.onmessage = function(event) {document.getElementById('hhtml').innerText = event.data2;};</script>";

  page += "<h3>Iluminação ambiente: <span id='luzhtml'>" + String(luzhtml) + "</span>%</h3>";
  page += "<script>var source = new EventSource('/sse'); source.onmessage = function(event) {document.getElementById('luzhtml').innerText = event.data3;};</script>";
  
  page += "</body></html>";
  return page;
}

String getUpdate() {
  return "data1: " + String(thtml) + "\n\n";
  return "data2: " + String(hhtml) + "\n\n";
  return "data3: " + String(luzhtml) + "\n\n";
}